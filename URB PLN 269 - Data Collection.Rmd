...
combined <- acs_data_tract %>% left_join(HCD_Data,by=c("county"="County"))
          # combined <- merge(acs_data_tract,HCD_Data,by.x="county",by.y="County",all.x=T)  # OLD way to join, doesn't preserve geometries like DPLYR.
...
```{r}
layers <- st_layers("Input/CES4.gdb")
layers

layers <- st_layers("Input/SB535.gdb")
layers
