...
#Get rid of geometry so we can use a DPLYR join
...
data <- combined %>% group_by(GEOID) %>% 
  mutate_each(funs(mean),Pollution,PollutionScore) %>%  
  distinct(GEOID,.keep_all=TRUE)

# Add tracts that are not DACs and are below one of the income limits
possibleDACs <- filter(data,LowIncomeTract==TRUE,
                is.na(DAC_category)==TRUE)
...
```{r}
data<- data %>% group_by(County) %>% mutate(Pollution_County = rank(PollutionScore),
...
sum(data$LOCALDAC,na.rm=T)  #THIS METHOD IDENTIFIES 591 NEW CENSUS TRACTS THAT WERE PREVIOUSLY NOT CONSIDERED!!!!!!!!

mapview(data,zcol="LOCALDAC")
